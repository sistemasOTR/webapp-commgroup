          ...
          <?php
            if(!empty($arrServicios))
            {
              ...
              foreach ($arrServicios as $key => $value) {
                ...
                $url_hist = $url_detalle."&fechaing=".$value->SERTT11_FECSER->format('Y-m-d')."&nroing=".$value->SERTT12_NUMEING."&nrodoc=".$value->SERTT31_PERNUMDOC."&empresa=".$user->getUserSistema();
