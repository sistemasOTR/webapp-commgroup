<?php
  ...
  $fmes = $entrega->getFechaEntrega()->format('m');
  $fdia = $entrega->getFechaEntrega()->format('d');
  $fyear = $entrega->getFechaEntrega()->format('Y');
  $ffechaAsig = $entrega->getFechaEntrega()->format('d-m-Y');
  ...
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <br>
        <p>Entre COMMERCIAL GROUP S.R.L., representada en este acto por su socio gerente, Sr. FRANCO DAVID ANFOSSI, DNI 32.903.094 según surge del contrato social inscripto al T 163, Fº 15.790, Nº 997, Sec. Contratos, Reg. Comercio de Rosario, con domicilio en calle San Luis 912, de Rosario, Provincia de Santa Fe, CUIT 30-71239507-5, en adelante llamada en adelante EL COMODANTE, y el Sr/a <?php echo $usuario->getApellido(); ?>, <?php echo $usuario->getNombre(); ?> titular del CUIL <?php echo $legajo_gestor->getCuit() ?> domiciliado en <?php echo $legajo_gestor->getDireccion() ?>, en adelante, el COMODATARIO, se conviene la firma del contrato que se regirá por las siguientes cláusulas:</p>
        <?php if($entrega->getIMEI()!=''){ ?>
