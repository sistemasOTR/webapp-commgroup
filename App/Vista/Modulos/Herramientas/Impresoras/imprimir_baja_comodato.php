    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <?php } elseif ($tipo == 'robo') { ?>
          <p>Se deja sin efecto el comodato a nombre del Sr/a. <?php echo $legajo_gestor->getNombre(); ?> titular del CUIL <?php echo $legajo_gestor->getCuit() ?> iniciado el día <?php echo $ffechaAsig; ?> referido a tal bien: Multifunción <span style="text-transform: uppercase;"><?php echo $impresora['_marca']; ?> <?php echo $impresora['_modelo']; ?></span> con Nº de serie <?php echo $impresora['_serialNro']; ?> siendo que ha sufrido un hecho de inseguridad (robo en la vía pública) mientras cumplía tareas laborales cuya denuncia en sede policial acredita la declaración del empleado y se adjunta a la presenta baja de comodato</p>
        <?php } elseif ($tipo == 'perdida') { ?>
          <p>Se deja sin efecto el comodato a nombre del Sr/a. <?php echo $legajo_gestor->getNombre(); ?> titular del CUIL <?php echo $legajo_gestor->getCuit() ?> iniciado el día <?php echo $ffechaAsig; ?> referido a tal bien: Multifunción <span style="text-transform: uppercase;"><?php echo $impresora['_marca']; ?> <?php echo $impresora['_modelo']; ?></span> con Nº de serie <?php echo $impresora['_serialNro']; ?> siendo que ha extraviado dicho bien.</p>
        <?php } ?>
