     ...
     $totales2 = 0;
        ...
        for ($i=6; $i >= 1 ; $i--) { 
            ...
            $anioMES = date('Y',mktime(0,0,0,date('m')-$i,1,date('Y')));
             ...
             $servCerrados = $handler->selectCountServicios($fdesde,$fhasta,200,$est_empresa,null,null,null,null);
             $cerrados2 += $servCerrados[0]->CANTIDAD_SERVICIOS;
             $servTotales = $handler->selectCountServicios($fdesde,$fhasta,null,$est_empresa,null,null,null,null);
             $totales2 += $servTotales[0]->CANTIDAD_SERVICIOS;
             // if (!empty($servTotales[0]->CANTIDAD_SERVICIOS)){
             //    $seguir='si';
             //  }else{
             //    $seguir='no';
             //  }
