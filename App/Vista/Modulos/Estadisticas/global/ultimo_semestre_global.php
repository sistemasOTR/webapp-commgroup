<?php
  ...
  // $handler =  new HandlerConsultas;
     ...
     $totalesg = 0;
        ...
        for ($i=6; $i >= 1 ; $i--) { 
            ...
            $anioMES = date('Y',strtotime($monthsMinus));
             ...
             $servCerrados = $handler->selectCountServiciosGestion($fdesde,$fhasta,200,null,null,null,null,null);
             $cerrados2 += $servCerrados[0]->CANTIDAD_SERVICIOS;
             $servTotales = $handler->selectCountServiciosGestion($fdesde,$fhasta,null,null,null,null,null,null);
             $totales2 += $servTotales[0]->CANTIDAD_SERVICIOS;
              if ($servTotales[0]->CANTIDAD_SERVICIOS != 0 && $servCerrados[0]->CANTIDAD_SERVICIOS != 0) {
                  $dataSemestreGlobal[] = array('mes' => $nombreMES,
    ...
    foreach ($dataSemestreGlobal as $key => $value) {
      $labelsg = $labelsg."'".$value['mes']."', ";
      $datag = $datag.$value['EFICIENCIA'].", "; 
...
?>
  ...
  <div class="box-body text-center">
    ...
    <canvas id="sem_chart_global" class="col-xs-12 chart"></canvas>
    <div class="col-xs-4 border-right">
      <h5>Cerradas</h5>
      <h3 class="text-light-blue"><?php echo $cerradosg; ?></h3>
    </div>
    <div class="col-xs-4 border-right">
      <h5>Total</h5>
      <h3 class="text-blue"><?php echo $totalesg; ?></h3>
    </div>
    <div class="col-xs-4">
      <h5>Eficiencia</h5>
      <h3 class="text-blue"><?php echo number_format($cerradosg*100/$totalesg,2); ?> %</h3>
    </div>
  </div>
</div>
...
<script>
  var config_sem_global = {
    type: 'line',
    data: {
      labels: [<?php echo $labelsg ?>],
      datasets: [{ 
        label: 'Efectividad',
        data: [<?php echo $datag ?>],
        fill: false,
