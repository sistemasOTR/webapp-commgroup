<?php
  ...
  $view_detalle= "index.php?view=puntajes_supervisor_detalle";
  ...
  <section class="content">
    ...
    <div class="row">
        ...
        for ($i=0; $i <= 5 ; $i++) { 
            ...
            if(!empty($arrPlazas))
            {                   
              foreach ($arrPlazas as $key => $value_plaza) {

                $consulta = $handler->consultaPuntajesCoordinador($fdesde, $fhasta, $value_plaza["alias"]);
                ...
                $objetivo_total = $objetivo_total + $objetivo;
              }

              if($objetivo_total != 0){
                if ($total_puntajes_enviadas > $objetivo_total) {
                  $clase_medidor = 'class="info-box bg-green"';
                  $puntajePorciento = round(($total_puntajes_enviadas - $objetivo_total)*100/$objetivo_total,2);
                  $txtPuntajePorciento = round($total_puntajes_enviadas * 100/$objetivo_total,2);
                } else {
                  $clase_medidor = 'class="info-box bg-yellow"';
                  $puntajePorciento = round(($total_puntajes_enviadas) * 100 /$objetivo_total,2);
                  $txtPuntajePorciento = round($total_puntajes_enviadas * 100/$objetivo_total,2);
                }
                ...
                <div <?php echo $clase_medidor; ?>>
                      ...
                      <div class="info-box-content">
                        ...
                        <span class="progress-description">
                          <small>Enviados: </small><?php echo $total_puntajes_enviadas ?> <span class="pull-right"><small>Objetivo: </small><?php echo $objetivo_total; ?></span>
                        </span>
                      </div>
                    </div>
                  </div>
...
<script type="text/javascript">        
  ...
  function crearHref()
  {
      ...
      url_filtro_reporte="index.php?view=puntajes_supervisor&fdesde="+f_inicio+"&fhasta="+f_fin;
